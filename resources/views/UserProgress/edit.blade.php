...
@section('content')
    <div class="container mt-5">
        ...
        <form action="{{ route('userProgress.update', $userProgress->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <input type="text" name="status" class="form-control" value="{{ $userProgress->status }}" required>
            </div>

            <div class="form-group">
                <label for="score">Score</label>
                <input type="number" name="score" class="form-control" value="{{ $userProgress->score }}">
            </div>

            <div class="form-group">
                <label for="completion_percentage">Completion Percentage</label>
                <input type="number" name="completion_percentage" class="form-control" value="{{ $userProgress->completion_percentage }}" step="0.01" min="0" max="100">
            </div>
